    ...
    >
      <Link href={`/ads/${ad.id}`} className="cursor-pointer block h-full">
        <div className="relative group">
          ...
          >
            <span
              className={`font-bold text-yellow-400 ${featured ? "text-lg" : "text-sm md:text-base"}`}
            >
              {ad.brand_name}
            </span>
          </motion.div>
        ...
        <div className={`p-4 relative z-10 ${featured ? "p-6" : "p-4"}`}>
          ...
          >
            <h3
              className={`font-bold text-yellow-400 font-[family-name:var(--font-teko)] tracking-wide leading-tight ${featured ? "text-2xl md:text-3xl" : "text-lg md:text-xl"}`}
            >
